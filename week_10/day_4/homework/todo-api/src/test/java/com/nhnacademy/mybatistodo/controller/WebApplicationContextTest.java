...
import com.nhnacademy.mybatistodo.advice.CommonControllerAdvice;
import com.nhnacademy.mybatistodo.config.RootConfig;
import com.nhnacademy.mybatistodo.config.WebConfig;
...
class WebApplicationContextTest {
    ...
    void getEventOfIdTest() throws Exception {
        mockMvc.perform(get("/api/calendar/events/1")
                ...
                .andExpect(jsonPath("$.subject").value("test"))//createAt 은 비교 불가.
                .andExpect(jsonPath("$.id").value("1"))
                .andExpect(jsonPath("$.eventAt").value("2023-05-01"))
                .andReturn();
    }
    ...
    void deleteEventOfIdTest() throws Exception {
        mockMvc.perform(delete("/api/calendar/events/4")
                        .header("X-USER-ID", "marco")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andReturn();
    ...
    void deleteEventOfEventIdTest() throws Exception {
        mockMvc.perform(delete("/api/calendar/events/daily/2023-05-01")
                        .header("X-USER-ID", "marco")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andReturn();
