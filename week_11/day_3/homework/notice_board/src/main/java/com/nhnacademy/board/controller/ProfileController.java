...
public class ProfileController {
    ...
    public String editUser(@ModelAttribute UserRequest userRequest) throws IOException {
        ...
        if (!Objects.equals(userRequest.getImage().getOriginalFilename(), "")) {
            fileName = userRequest.getId() + JPEG;
            userRequest.getImage().transferTo(Paths.get(UPLOAD_DIR + IMAGE + File.separator + fileName));
        }

        userService.modifyUser(new UserEditDto(userRequest, fileName));
        return "redirect:/admin/view-user/" + userRequest.getId();
    }
}
